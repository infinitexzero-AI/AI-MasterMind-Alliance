...
export default function FleetDashboard() {
  ...
  const vanguardNodes = hardwareStats?.vanguard || [];
  ...
  return (
    <NexusLayout>
      ...
      <div className="max-w-7xl mx-auto p-6 md:p-12">
        ...
        <div className="grid grid-cols-1 md:grid-cols-4 gap-6 mb-12">
          <div className="renaissance-panel p-6 border-l-4 border-indigo-500">
            ...
            <div className="text-4xl font-bold text-slate-100 mb-2">
              {activeNodes.length + vanguardNodes.length}
            </div>
            <div className="text-xs text-indigo-400 font-mono">
              Docker & Hardware Nodes
            </div>
          </div>
        ...
        {/* Swarm Hardware Radar (Vanguard Nodes) */}
        <div className="renaissance-panel p-8 mb-12">
          <h2 className="text-xl font-bold text-slate-100 mb-6 flex items-center">
            <Server className="w-5 h-5 mr-3 text-amber-400" />
            Vanguard Swarm Hardware
          </h2>

          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {vanguardNodes.length > 0 ? vanguardNodes.map((node: any, idx: number) => (
              <div key={idx} className="bg-slate-800/50 rounded-xl p-5 border border-amber-500/10 hover:border-amber-500/30 transition-colors shadow-lg shadow-amber-500/5">
                <div className="flex justify-between items-start mb-4">
                  <div className="flex items-center space-x-3">
                    ...
                    <div>
                      <h3 className="text-sm font-bold text-slate-200">{node.node_name || 'Vanguard_Node'}</h3>
                      <span className={`text-[10px] font-mono px-2 py-0.5 rounded ${node.status === 'online' ? 'bg-emerald-500/10 text-emerald-400' : 'bg-red-500/10 text-red-400'}`}>
                ...
                <div className="space-y-3">
                  <div>
                    <div className="flex justify-between text-xs text-slate-400 mb-1">
                      ...
                      <span>{node.cpu_percent || node.cpu || '0'}%</span>
                    </div>
                    <progress 
                      max="100" 
                      value={Math.min(parseFloat(node.cpu_percent || node.cpu || '0'), 100)}
                      className="w-full h-1.5 rounded-full overflow-hidden [&::-webkit-progress-bar]:bg-slate-700 [&::-webkit-progress-value]:bg-amber-500 [&::-moz-progress-bar]:bg-amber-500 bg-slate-700" 
                    />
                  </div>
                  <div>
                    <div className="flex justify-between text-xs text-slate-400 mb-1">
                      <span>Memory Utilization</span>
                      <span>{node.ram_percent || node.memory || '0'}%</span>
                    </div>
                    <progress 
                      max="100" 
                      value={Math.min(parseFloat(node.ram_percent || node.memory || '0'), 100)}
                      className="w-full h-1.5 rounded-full overflow-hidden [&::-webkit-progress-bar]:bg-slate-700 [&::-webkit-progress-value]:bg-orange-500 [&::-moz-progress-bar]:bg-orange-500 bg-slate-700" 
                    />
                  </div>
                  <div className="pt-2 border-t border-white/5 flex justify-between items-center">
                    <span className="text-[10px] text-slate-500 font-mono">Uptime: {node.uptime || 'N/A'}</span>
                    <span className="text-[10px] text-slate-500 font-mono">{node.temp ? `${node.temp}°C` : ''}</span>
                  </div>
                </div>
              </div>
            )) : (
              <div className="col-span-1 md:col-span-3 py-12 text-center text-slate-500 font-mono text-sm border-2 border-dashed border-white/5 rounded-xl">
                Waiting for Vanguard hardware check-in...<br/>
                <span className="text-xs text-slate-600 mt-2 block">Ensure nodes are running the vanguard_bridge daemon targeting this relay.</span>
              </div>
            )}
          </div>
        </div>
