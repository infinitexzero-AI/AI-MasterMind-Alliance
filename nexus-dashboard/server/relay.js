...
const redisSubscriber = { on: () => {}, subscribe: () => {} };

// In-memory fallback for system state (when Redis is bypassed or unreachable)
const localCache = {
  clipboard: { text: '', source: 'system', timestamp: new Date().toISOString() }
};

console.log('--- RELAY CORE INITIALIZING (REDIS BYPASS MODE) ---');
...
const EVENT_BUS_LOG = fs.existsSync(path.join(AILCC_ROOT, '06_System/Logs/event_bus.jsonl')) 
  ? path.join(AILCC_ROOT, '06_System/Logs/event_bus.jsonl')
  : path.join(AILCC_ROOT, '06_System/Logs/system_relay.log');
...
async function synthesizeNarrative(details) {
  ...
  const span = { update: () => {}, end: () => {} }; // Mocked for stability
  // opik.span({
  ...
  try {
    ...
    span.update({ output: { error: err.message }, metadata: { status: "failed" } });
...
app.get('/api/mobile/briefing', async (req, res) => {
  try {
    ...
    try {
      strategicInsight = execSync(`python3 ${path.join(AILCC_ROOT, '06_System/Execution/strategic_briefing_gen.py')}`, { encoding: 'utf8' }).trim();
    } catch (e) {
...
// API: Cross-Platform Clipboard Sync (Phase 27 - LocalDrop Protocol)
app.post('/api/system/clipboard', async (req, res) => {
  const { action, text, source = 'unknown' } = req.body;
  ...
  try {
    ...
    if (action === 'set') {
      if (typeof text !== 'string') return res.status(400).json({ error: 'Text required for set' });
      
      // Prevent redundant updates if text is the same
      if (localCache.clipboard.text === text && localCache.clipboard.source === source) {
        return res.json({ success: true, status: 'unchanged', timestamp: localCache.clipboard.timestamp });
      }

      // 1. Update Local Memory Cache
      localCache.clipboard = { text, source, timestamp };

      // 2. Update Redis (Async fire-and-forget, handles failures silently if bypassed)
      if (process.env.NEXT_PUBLIC_RELAY_BYPASS !== 'true') {
        redis.hset('ailcc:state', 'clipboard', text).catch(() => {});
        redis.hset('ailcc:state', 'clipboard_source', source).catch(() => {});
        redis.hset('ailcc:state', 'clipboard_ts', timestamp).catch(() => {});
      }

      // 3. Broadcast to all UI clients and bridge daemons
      // Using both for transition compatibility
      io.emit('CLIPBOARD_SYNC', { text, source, timestamp });
      io.emit('CLIPBOARD_UPDATE', { text, source, timestamp });
      
      console.log(`📡 [Sync Hub] Clipboard updated from: ${source} (Size: ${text.length} chars)`);
      return res.json({ success: true, timestamp });
      
    } else if (action === 'get') {
      // Try Redis first, then fallback to local cache
      let currentText = localCache.clipboard.text;
      let currentSource = localCache.clipboard.source;
      let currentTimestamp = localCache.clipboard.timestamp;

      if (process.env.NEXT_PUBLIC_RELAY_BYPASS !== 'true') {
        try {
          const val = await redis.hget('ailcc:state', 'clipboard');
          const src = await redis.hget('ailcc:state', 'clipboard_source');
          const ts = await redis.hget('ailcc:state', 'clipboard_ts');
          if (val) {
            currentText = val;
            currentSource = src || 'redis';
            currentTimestamp = ts || timestamp;
            
            // Sync local cache with Redis value if it's newer/different
            if (localCache.clipboard.text !== currentText) {
                localCache.clipboard = { text: currentText, source: currentSource, timestamp: currentTimestamp };
            }
          }
        } catch (e) {
          // Fallback handled by initialized variables
        }
      }

      return res.json({ text: currentText, source: currentSource, timestamp: currentTimestamp });
    } else {
...
// API: Node Heartbeat (for Swarm Observability)
app.post('/api/system/nodes/heartbeat', async (req, res) => {
  const nodeData = req.body;
  if (!nodeData.node_name) return res.status(400).json({ error: 'node_name required' });
  ...
  try {
    // Store in Redis (Mesh Nodes)
    await redis.hset('ailcc:mesh:nodes', nodeData.node_name, JSON.stringify(nodeInfo));
    
    // Broadcast to Dashboard immediately
    io.emit('NODE_HEARTBEAT', nodeInfo);
    
    // Also trigger a full hardware telemetry broadcast
    broadcastHeartbeat();
...
// --- TASK LOG AGGREGATION (V1 COMPATIBILITY) ---
app.get('/api/v1/logs', (req, res) => {
    ...
    try {
        const content = fs.readFileSync(EVENT_BUS_LOG, 'utf8');
        let logs = content.split('\n')
            .filter(line => line.trim())
            .map(line => {
                try { return JSON.parse(line); } 
                catch { return { timestamp: new Date().toISOString(), type: 'LOG', message: line }; }
            });

        if (taskId) {
            logs = logs.filter(l => l.taskId === taskId || l.payload?.taskId === taskId);
        }
...
app.post('/api/v1/swarms/:taskId/approve', async (req, res) => {
    const { taskId } = req.params;
    console.log(`✅ Manual Approval received for Task: ${taskId}`);
    
    try {
        await redis.publish('NEURAL_SYNAPSE', JSON.stringify({
            type: 'HUMAN_APPROVAL',
            taskId,
            timestamp: new Date().toISOString()
        }));
        res.json({ status: 'approved', taskId });
    } catch (err) {
        res.status(500).json({ error: 'Approval dispatch failed' });
    }
});
...
app.post('/api/comet/ingest', (req, res) => {
  ...
  //   name: "comet_ingest",
  //   input: { source, topic, data_size: JSON.stringify(data).length },
  ...
  try {
    ...
    trace.update({ output: { status: "success", path: fullPath } });
    res.status(201).json({ status: 'success', path: fullPath });
  } catch (err) {
    trace.update({ output: { error: err.message }, metadata: { status: "failed" } });
...
const PORT = process.env.PORT || process.env.RELAY_PORT || 5005;
