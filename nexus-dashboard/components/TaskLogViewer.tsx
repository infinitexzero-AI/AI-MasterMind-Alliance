...
export function TaskLogViewer({ taskId, limit = 20, refreshInterval = 3000, title = "Protocol Stream" }: TaskLogViewerProps) {
    const url = `http://localhost:3001/api/v1/logs?limit=${limit}${taskId ? `&taskId=${taskId}` : ''}`;
    ...
    const handleApprove = async (log: FlattenedLog) => {
        ...
        try {
            await fetch(`http://localhost:3001/api/v1/swarms/${log.taskId}/approve`, { method: 'POST' });
        } catch (e) {
            console.error(e);
