...
export function AntigravityPanel() {
    ...
    useEffect(() => {
        ...
        socket.on('connect', () => {
            console.log('⚡ Antigravity Linked to Neural Relay (Port 3001)');
            setIsConnected(true);
        });

        socket.on('disconnect', () => {
            console.warn('⚠️ Antigravity Drift: Connection lost');
            ...
            try {
                ...
                if (data.type === 'CREW_STATUS') {
                    ...
                    setAgents(prev => prev.map(a => {
                        ...
                        if (status === 'WORKING') return { ...a, status: 'thinking' };
                        return a;
                    }));
                ...
                console.error('[Antigravity] Synapse Parse Error', e);
            }
        });

        // Also listen for generic system commands
        socket.on('SYSTEM_COMMAND', (data) => {
            console.log('📡 System Command over Relay:', data);
        });
    ...
    const handleTouchAction = (action: string) => {
        ...
        if (action === 'comet' || action === 'launch') {
            ...
            if (socketRef.current?.connected) {
                socketRef.current.emit('CREW_START', {
                    task: task,
                    context: 'User requested via Antigravity UI'
                });
            } else {
                console.warn("Neural Relay not ready");
            }
        ...
        if (action === 'design') {
            ...
            if (socketRef.current?.connected) {
                socketRef.current.emit('PING_REQUEST', {});
            }
        }
    };
        ...
        <div className="relative w-full h-[600px] overflow-hidden bg-gradient-to-br from-gray-900 via-black to-slate-900 rounded-3xl border border-white/10 shadow-2xl flex">
            ...
            <div className="absolute top-4 right-4 z-40 flex flex-col items-end gap-2">
                <div className="flex items-center gap-2 bg-black/40 backdrop-blur rounded-full px-3 py-1 border border-white/10">
                    ...
                    <span className="text-[10px] font-mono text-white/60">SYSTEM: {isConnected ? 'ONLINE' : 'LINK LOST'}</span>
                </div>
                {storage && (
