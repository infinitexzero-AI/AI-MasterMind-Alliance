...
export class OpenAIAdapter {
  ...
  async executeTask(handoff: HandoffContext): Promise<DispatchResult> {
    ...
    try {
      ...
      return {
        ...
        agentUsed: 'openai',
        output: response.choices[0].message?.content?.trim() || response.choices[0].text?.trim() || '',
        metadata: {
  ...
  private async callOpenAIAPI(userMessage: string): Promise<any> {
    ...
    // Normalize response to match expected output structure or return data directly
    // The executeTask expects response.choices[0].text, but Chat API returns choices[0].message.content
    // We will adjust executeTask to handle this.
    return data;
  }
