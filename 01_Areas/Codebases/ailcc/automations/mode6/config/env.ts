...
export class ConfigLoader {
  ...
  private loadConfig(): AgentConfig {
    return {
      ...
      xai: {
        ...
        model: process.env.GROK_MODEL || 'grok-4.20-experimental-beta-0304-reasoning',
      ...
      perplexity: {
        ...
        maxTokens: parseInt(process.env.PERPLEXITY_MAX_TOKENS || '4096', 10),
        temperature: parseFloat(process.env.PERPLEXITY_TEMPERATURE || '0.7'),
        mockMode: !process.env.PERPLEXITY_API_KEY,
      },
    };
  }
   ...
   */
  isAgentMocked(agent: 'anthropic' | 'openai' | 'xai' | 'gemini' | 'perplexity'): boolean {
   ...
   */
  getAvailableAgents(): Array<'anthropic' | 'openai' | 'xai' | 'gemini' | 'perplexity'> {
    const available: Array<'anthropic' | 'openai' | 'xai' | 'gemini' | 'perplexity'> = [];
    ...
    if (!this.config.gemini.mockMode) available.push('gemini');
    if (!this.config.perplexity.mockMode) available.push('perplexity');
    return available;
  }
  ...
  logStatus(): void {
    ...
    console.log(`  • Google Gemini: ${this.config.gemini.mockMode ? '⚠️  MOCK MODE' : '✅ Ready'}`);
    console.log(`  • Perplexity: ${this.config.perplexity.mockMode ? '⚠️  MOCK MODE' : '✅ Ready'}`);
  }
}
