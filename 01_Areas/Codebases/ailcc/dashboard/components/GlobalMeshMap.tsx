...
export default function GlobalMeshMap() {
  ...
  useEffect(() => {
    // Attempt connecting to the WS relay for live mesh updates
    const ws = new WebSocket(`ws://localhost:5005/ws`);
    ws.onmessage = (event) => {
      try {
        const data = JSON.parse(event.data);
        if (data.channel === 'NEURAL_SYNAPSE') {
          const payload = JSON.parse(data.message);
          if (payload.intent === 'SWARM_SCALING_PROPOSED') {
             setSynapseLog(`LEGION ALERT: ${payload.details.action} | Const: ${payload.details.cost}`);
