...
import { ClaudeAgent } from '../ai-providers/claude';
...
export class CodeOrchestrator {
  ...
  constructor(
    ...
    this.parser = new TaskParser(anthropicKey);
    this.claude = new ClaudeAgent(anthropicKey);
  ...
  async plan(prompt: string): Promise<CodeGenerationTask[]> {
    // Use Parser (Claude) if available, otherwise switch to Copilot (OpenAI)
  ...
  assignAgent(task: CodeGenerationTask): AgentType {
    const claudeOk = process.env.ANTHROPIC_API_KEY && !process.env.ANTHROPIC_API_KEY.includes('placeholder');
    
    // If Claude is requested but missing, route to Copilot
    if (task.assignedTo === 'claude' && !claudeOk) {
        return 'copilot';
    }

    // Original Heuristics
    if (task.complexity === 'complex' && claudeOk) return 'claude';
    ...
    // Default fallback
    return claudeOk ? 'claude' : 'copilot';
  }
  ...
  async executeTask(task: CodeGenerationTask): Promise<CodeGenerationTask> {
    ...
    switch (agentType) {
      case 'claude': agent = this.claude; break;
