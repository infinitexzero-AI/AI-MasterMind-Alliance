...
# Optimization: Enforces background priority and root-relative pathing.

# 0. Environment Setup
AILCC_ROOT="${AILCC_ROOT:-$HOME/AILCC_PRIME}"
LOGS_DIR="$AILCC_ROOT/logs"
SCRIPTS_DIR="$AILCC_ROOT/scripts"
...
# Helper for throttled ps/du
function throttled_ps() {
    taskpolicy -c background nice -n 19 ps "$@"
}

function throttled_du() {
    taskpolicy -c background nice -n 19 du "$@"
}

# 1. Monitor Language Server
echo "[1/4] Language Server Health:"
LS_PID=$(throttled_ps aux | grep language_server_macos | grep -v grep | awk '{print $2}' | head -1)

if [ -n "$LS_PID" ]; then
    LS_MEM_PERCENT=$(throttled_ps aux | grep language_server_macos | grep -v grep | awk '{print $4}' | head -1)
    ...
    if [ "$LS_MEM_INT" -gt 30 ]; then
        ...
        bash "$SCRIPTS_DIR/restart_language_server.sh"
        echo "  ✓ Restart initiated"
        
        # Log restart
        echo "$(date +%Y-%m-%d\ %H:%M:%S),language_server,${LS_MEM_PERCENT}%,restarted" >> "$LOGS_DIR/process_health.log"
    else
        echo "  ✅ Healthy"
    fi
else
...
DOCKER_RUNNING=$(throttled_ps aux | grep -i docker | grep -v grep | wc -l | tr -d ' ')
...
if [ "$DOCKER_RUNNING" -gt 0 ]; then
    ...
    if [ -f "$DOCKER_RAW_PATH" ]; then
        DOCKER_RAW_GB=$(throttled_du -sk "$DOCKER_RAW_PATH" 2>/dev/null | awk '{print int($1/1024/1024)}')
        ...
        if [ -n "$DOCKER_RAW_GB" ]; then
            ...
            if [ "$DOCKER_RAW_GB" -gt 50 ]; then
                ...
                echo "$(date +%Y-%m-%d\ %H:%M:%S),docker,${DOCKER_RAW_GB}GB,critical" >> "$LOGS_DIR/process_health.log"
                ...
                echo "$(date +%Y-%m-%d\ %H:%M:%S),docker,${DOCKER_RAW_GB}GB,warning" >> "$LOGS_DIR/process_health.log"
            else
                echo "  ✅ Healthy"
            fi
        fi
    ...
    DOCKER_VM_MEM=$(throttled_ps aux | grep "com.apple.Virtualization.VirtualMachine" | grep -v grep | awk '{print $4}' | head -1)
...
CHROME_COUNT=$(throttled_ps aux | grep -i chrome | grep -v grep | wc -l | tr -d ' ')

if [ "$CHROME_COUNT" -gt 0 ]; then
    CHROME_TOTAL_MEM=$(throttled_ps aux | grep -i chrome | grep -v grep | awk '{sum+=$4} END {print sum}')
    ...
    if [ "$CHROME_COUNT" -gt 30 ]; then
        ...
        echo "$(date +%Y-%m-%d\ %H:%M:%S),chrome,${CHROME_COUNT} processes,warning" >> "$LOGS_DIR/process_health.log"
    else
        echo "  ✅ Healthy"
    fi
else
...
ANTIGRAVITY_COUNT=$(throttled_ps aux | grep "Antigravity Helper" | grep -v grep | wc -l | tr -d ' ')

if [ "$ANTIGRAVITY_COUNT" -gt 0 ]; then
    ANTIGRAVITY_TOTAL_MEM=$(throttled_ps aux | grep "Antigravity Helper" | grep -v grep | awk '{sum+=$4} END {print sum}')
